...
-----
    ...
    --data-dir   DIR   directory containing train/validation/test parquet files
                       (default: data)
    --output-dir DIR   directory for plots and CSV outputs  (default: outputs)
...
def parse_args() -> dict:
    ...
    p.add_argument("--output-dir", default="outputs", help="Plots and CSV outputs")
...
def train_nlp_classifier(
    ...
    if cfg.get("plots", True):
        ...
        plt.tight_layout()
        out = Path(cfg["output_dir"]) / "nlp_confusion_matrix.png"
...
def run_pca(transitions: pd.DataFrame, cfg: dict) -> Tuple[np.ndarray, List[str]]:
    ...
    if cfg.get("plots", True):
        ...
        plt.tight_layout()
        out = Path(cfg["output_dir"]) / "pca_sector_profiles.png"
...
def compute_drift_and_clusters(
    ...
    out = Path(output_dir) / "drift_risk_scores.csv"
...
def compute_association_rules(
    ...
    out = Path(output_dir) / "association_rules.csv"
...
def plot_sector_distribution(df: pd.DataFrame, output_dir: str) -> None:
    ...
    plt.tight_layout()
    _save(fig, Path(output_dir) / "sector_distribution.png")
...
def plot_sequence_length(df: pd.DataFrame, output_dir: str) -> None:
    ...
    plt.tight_layout()
    _save(fig, Path(output_dir) / "sequence_length_distribution.png")
...
def plot_drift_distribution(risk_df: pd.DataFrame, output_dir: str) -> None:
    ...
    ax.set_ylabel("Number of Users")
    plt.tight_layout()
    _save(fig, Path(output_dir) / "career_drift_distribution.png")
...
def plot_transition_heatmap(transitions: pd.DataFrame, df: pd.DataFrame, output_dir: str) -> None:
    ...
    plt.yticks(fontsize=7)
    plt.tight_layout()
    _save(fig, Path(output_dir) / "transition_heatmap.png")
...
def plot_top_transitions(transitions: pd.DataFrame, output_dir: str) -> None:
    ...
    ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{int(x):,}"))
    plt.tight_layout()
    _save(fig, Path(output_dir) / "top_transitions.png")
...
def plot_market_drift(market_drift: dict, output_dir: str) -> None:
    ...
    plt.tight_layout()
    _save(fig, Path(output_dir) / "market_drift.png")
...
def plot_user_clusters(risk_df: pd.DataFrame, cs2: pd.DataFrame, output_dir: str) -> None:
    ...
    plt.tight_layout()
    _save(fig, Path(output_dir) / "user_clusters.png")
...
def plot_cluster_sector_breakdown(risk_df: pd.DataFrame, cs2: pd.DataFrame,
    ...
    plt.tight_layout()
    _save(fig, Path(output_dir) / "cluster_sector_breakdown.png")
...
def plot_accuracy_comparison(
    ...
    plt.tight_layout()
    _save(fig, Path(output_dir) / "accuracy_comparison.png")
...
def plot_association_rules_lift(assoc_rules: pd.DataFrame, output_dir: str) -> None:
    ...
    ax.set_xlabel("Lift")
    plt.tight_layout()
    _save(fig, Path(output_dir) / "association_rules_lift.png")
...
def print_summary(
    ...
    OUTPUT_FILES = [
        "nlp_confusion_matrix.png", "sector_distribution.png",
        ...
        "association_rules_lift.png", "drift_risk_scores.csv",
        "association_rules.csv",
    ]
    log.info("Output files:")
    for f in OUTPUT_FILES:
        exists = (Path(output_dir) / f).exists()
