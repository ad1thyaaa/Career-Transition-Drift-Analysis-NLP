"""
Career Drift Trajectory Analysis — Production Pipeline
=======================================================
...
-----
    ...
    --data-dir   DIR   directory containing train/validation/test parquet files
                       (default: data)
    --output-dir DIR   root output directory; figures go to <output-dir>/figures,
                       tables to <output-dir>/tables, logs to <output-dir>/logs
                       (default: results)
    --cache-dir  DIR   directory for embedding / model caches (default: cache)
    --model-dir  DIR   directory for saved model checkpoints  (default: models)
    --seed       INT   global random seed                     (default: 42)
    --no-cache        force re-computation even if caches exist
    --no-plots        skip all matplotlib visualisation steps

Output layout (auto-created):
    results/figures/   all matplotlib / seaborn plots
    results/tables/    all CSV / tabular outputs
    results/logs/      pipeline.log (mirrors terminal output)
    cache/             parquet checkpoints and embedding .npy files
    models/            LightGBM .joblib checkpoints

Run sections individually by importing the module and calling any stage
function directly.
"""
...
log = logging.getLogger("career_drift")
...
def make_dirs(cfg: dict) -> None:
    """Create all required output, cache, and model directories."""
    # Top-level directories for cache and model checkpoints
    for key in ("cache_dir", "model_dir"):
        Path(cfg[key]).mkdir(parents=True, exist_ok=True)

    # Structured output sub-directories
    out = Path(cfg["output_dir"])
    (out / "figures").mkdir(parents=True, exist_ok=True)   # all matplotlib / seaborn plots
    (out / "tables").mkdir(parents=True, exist_ok=True)    # all CSV / tabular outputs
    (out / "logs").mkdir(parents=True, exist_ok=True)      # pipeline.log file


def setup_file_logging(cfg: dict) -> None:
    """
    Attach a FileHandler to the root logger so every log record is
    mirrored to results/logs/pipeline.log in addition to the terminal.
    Must be called after make_dirs() so the directory already exists.
    """
    log_path = Path(cfg["output_dir"]) / "logs" / "pipeline.log"
    ...
    log.info("File logging active → %s", log_path)


def parse_args() -> dict:
    p = argparse.ArgumentParser(description="Career Drift Trajectory Pipeline")
    p.add_argument("--data-dir",   default="data",    help="Parquet input directory")
    p.add_argument("--output-dir", default="results", help="Root output directory (figures/tables/logs auto-created inside)")
    p.add_argument("--cache-dir",  default="cache",   help="Embedding / array caches")
    p.add_argument("--model-dir",  default="models",  help="Saved model checkpoints")
    p.add_argument("--seed",       type=int, default=42)
    p.add_argument("--no-cache",   action="store_true", help="Force re-computation")
    p.add_argument("--no-plots",   action="store_true", help="Skip visualisations")
    args = p.parse_args()
    out = args.output_dir
    return {
        "data_dir":   args.data_dir,
        "output_dir": out,
        # Convenience sub-directory paths (pathlib.Path objects)
        "fig_dir":    Path(out) / "figures",   # all matplotlib / seaborn figures
        "tbl_dir":    Path(out) / "tables",    # all CSV / tabular outputs
        "log_dir":    Path(out) / "logs",      # pipeline.log
...
def load_data(data_dir: str) -> pd.DataFrame:
    """Load and concatenate train / validation / test parquet files."""
    dp = Path(data_dir)
    ...
    for split in ("train", "validation", "test"):
        ...
        log.info("  Loaded %-12s  %s rows", split, f"{len(frames[-1]):,}")
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    """Remove unknowns, duplicates, and out-of-range years. Sort by person+year."""
    ...
    log.info("  Jobs       : %s", f"{df['matched_label'].nunique():,}")
...
# ---------------------------------------------------------------------------
# 3a.  map_sectors() — detailed expert rules (ported from R map_sectors)
# ---------------------------------------------------------------------------
...
def map_sectors(txt: str) -> str:
    ...
    Applies detailed expert regex patterns to lowercased text.
    First match wins (replicates R fcase semantics exactly).
    """
...
# ---------------------------------------------------------------------------
# 3b.  broad_map() — broad fallback keyword rules (ported from R broad_map)
# ---------------------------------------------------------------------------
...
_BROAD_SIMPLE: List[Tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"accountant|auditor|payroll|bookkeep|treasury|\bcfo\b|financial controller|fp.a|accounts payable"),
     "Finance & Accounting"),
    # index 5 — compound sales rule inserted in broad_map()
...
def broad_map(txt: str) -> str:
    """
    Port of R broad_map().
    Broad keyword fallback; compound conditions are replicated explicitly.
    Order exactly matches R fcase() evaluation sequence.
    """
    # Indices 0–4 (before compound sales in R)
    for pat, label in _BROAD_SIMPLE[:5]:
        if pat.search(txt):
            return label
    # Compound sales (R position 6)
    if _BROAD_SALES_KW.search(txt) and _BROAD_SALES_ROLE.search(txt):
        return "Sales & Business Development"
    # Indices 5–24 (simple rules continuing after sales)
    for pat, label in _BROAD_SIMPLE[5:-1]:
        if pat.search(txt):
            return label
    # Compound creative/media (second-to-last in R)
...
def apply_regex_pipeline(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Pass 2b : broad_map() on label then description for rows still 'Other'
    Pass 3  : drop rows still 'Other & Unclassified'

    Adds column  esco_sector_regex  to df.
    Returns a new DataFrame (does not modify in place).
    """
    log.info("--- WEAK SUPERVISION: 4-pass regex bootstrap ---")
...
def _embed(
    ...
    """Generate or load cached L2-normalised SBERT embeddings."""
    if use_cache and cache_path and cache_path.exists():
        log.info("  Loading cached embeddings: %s", cache_path)
        return np.load(cache_path)
    log.info("  Encoding %s (%s texts) …", label, f"{len(texts):,}")
    emb = model.encode(
        texts,
...
def train_nlp_classifier(
    ...
    """
    Train SBERT + LightGBM sector classifier using weak-supervision labels.
    ...
    """
    log.info("--- NLP CLASSIFIER: SBERT + LightGBM ---")
    seed       = cfg["seed"]
    ...
    X_train = _embed(train_t, embedding_model, cache_dir / "emb_train.npy", use_cache, "train")
    X_val   = _embed(val_t,   embedding_model, cache_dir / "emb_val.npy",   use_cache, "val")
    X_test  = _embed(test_t,  embedding_model, cache_dir / "emb_test.npy",  use_cache, "test")
    ...
    if use_cache and ckpt_path.exists():
        ...
        log.info("  Training LightGBM classifier on %s bootstrap labels …", f"{len(train_t):,}")
    ...
    if cfg.get("plots", True):
        ...
        ax.set_title("NLP Sector Classification — Confusion Matrix\n(vs bootstrap regex labels)",
                     fontsize=12, fontweight="bold")
        plt.xticks(rotation=60, ha="right", fontsize=7)
        plt.yticks(fontsize=7)
        plt.tight_layout()
        out = cfg["fig_dir"] / "nlp_confusion_matrix.png"
...
def predict_nlp_sectors(
    ...
    """
    Apply the NLP classifier to the full dataset and assign final esco_sector.
    Rows below the confidence threshold are dropped (mirrors R Pass 3).
    """
    log.info("--- PREDICTING FINAL NLP SECTORS ---")
    cache_dir = Path(cfg["cache_dir"])
    ckpt      = cache_dir / "emb_full.npy"
    ...
    df = df.copy()
    df["esco_sector"] = [
        s if p >= confidence_threshold else "Other & Unclassified"
    ...
    n_differ  = (df["esco_sector_regex"] != df["esco_sector"]).sum()
    log.info("  Regex vs NLP agreement : %.2f%%  (%s cases differ)", agree * 100, f"{n_differ:,}")
    if n_differ > 0:
        pairs = (
    ...
    df = df.sort_values(["person_id", "year"]).reset_index(drop=True)
    ckpt_df = Path(cfg["cache_dir"]) / "df_nlp_sectors.parquet"
    df.to_parquet(ckpt_df, index=False)
    log.info("  Checkpoint saved → %s", ckpt_df)
    return df
...
def build_transitions(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, Dict]:
    """
    Create next_sector / prev_sector columns, university flag, stickiness multipliers.
    Returns (df_with_flags, transitions_df, person_univ_dict).
    """
    log.info("--- TRANSITION MODELING ---")
    df = df.sort_values(["person_id", "year"]).reset_index(drop=True)
    ...
    log.info("  Users with >= 2 jobs    : %s", f"{(seq_stats['n_jobs'] >= 2).sum():,}")
    log.info("  Median sequence length  : %.1f", seq_stats["n_jobs"].median())
    ...
    if "university_studies" in df.columns:
        ...
        log.info("  university_studies column absent — flag set False for all")
...
def compute_stickiness(
    ...
    """Compute per-sector self-loop rates and stickiness multipliers."""
    sr = (
        transitions
        .assign(is_self=lambda x: x["esco_sector"] == x["next_sector"])
        .groupby("esco_sector")["is_self"].mean()
    )
    self_rates = {s: float(sr.get(s, 0.2)) for s in all_sectors}
    log.info("  Top 5 stickiest sectors:")
    for nm in sorted(self_rates, key=self_rates.get, reverse=True)[:5]:
        log.info("    %-45s : %.3f", nm, self_rates[nm])
    return self_rates
...
def run_pca(transitions: pd.DataFrame, cfg: dict) -> Tuple[np.ndarray, List[str]]:
    ...
    log.info("--- PCA: SECTOR TRANSITION PROFILES ---")
    ...
    log.info("  Variance — PC1:%.1f%%  PC2:%.1f%%  Cum:%.1f%%",
    ...
    pca_df           = pd.DataFrame({"PC1": pca_result[:, 0], "PC2": pca_result[:, 1], "sector": all_pca})
    ...
    if cfg.get("plots", True):
        ...
        plt.tight_layout()
        out = cfg["fig_dir"] / "pca_sector_profiles.png"
...
def stratified_user_split(
    ...
    log.info("  Building stratified user split (64 / 16 / 20) …")
    user_dom = (
...
def build_ensemble_components(
    ...
    """
    log.info("--- ENSEMBLE: BUILDING COMPONENTS ---")
    ...
    for start, end in decade_windows:
        ...
        if len(wd) == 0:
            log.info("  %-15s | %10s | Empty", lbl, "N/A")
            continue
    ...
    for j in range(1, len(decade_keys)):
        diff = (transition_matrices[decade_keys[j]]["mat"]
                - transition_matrices[decade_keys[j - 1]]["mat"])
        market_drift[decade_keys[j]] = float(np.sqrt((diff ** 2).sum()))
    ...
    log.info("  2nd-order keys: %s | Back-off: %s", f"{len(second_order_probs):,}", f"{n_backoff:,}")
    del so_data, bpc, btr
    gc.collect()

    # 8.7 Per-user history priors (HIST_DECAY = 0.15)
    HIST_DECAY = 0.15
    ...
    log.info("\n%s", cs_arch[["cluster", "archetype", "mean_drift", "mean_loop"]].round(4).to_string(index=False))
...
def evaluate_ensemble(
    ...
    log.info("  %s", "-" * 55)
    log.info("  %-22s | %6.2f%% | %7s | %7s", "Persistence Baseline",
             persistence_acc * 100, "N/A", "N/A")
    log.info("  %-22s | %6.2f%% | %6.2f%% | %6.2f%%", "Ensemble",
             results["top_1"] * 100, results["top_3"] * 100, results["top_5"] * 100)
...
# ===========================================================================
# 8.  DRIFT ANALYSIS & CLUSTERING  (full dataset)
# ===========================================================================
...
def compute_drift_and_clusters(
    ...
    Returns (risk_df, cs2).
    """
    log.info("--- DRIFT ANALYSIS & CLUSTERING ---")
    ...
    labels = km2.fit_predict(X_c)
    risk_df.loc[vi, "cluster"] = labels
    ...
    log.info("  Cluster summary:\n%s",
             cs2[["cluster", "archetype", "n_users", "mean_drift", "mean_loop"]].round(4).to_string(index=False))

    out = Path(output_dir) / "tables" / "drift_risk_scores.csv"
    risk_df.to_csv(out, index=False)
    log.info("  Saved: %s", out)
    log.info("  TOP 10 HIGHEST DRIFT RISK:\n%s",
...
def compute_association_rules(
    ...
    """Direct support / confidence / lift computation (mirrors R exactly)."""
    log.info("--- ASSOCIATION RULES ---")
    tot = len(transitions)
    ...
    log.info("  TOP 15 BY CONFIDENCE:\n%s",
             rules.sort_values("confidence", ascending=False).head(15).to_string(index=False))
    log.info("  TOP 15 BY SUPPORT:\n%s",
             rules.sort_values("support", ascending=False).head(15).to_string(index=False))

    out = Path(output_dir) / "tables" / "association_rules.csv"
...
def plot_sector_distribution(df: pd.DataFrame, output_dir: str) -> None:
    ...
    plt.tight_layout()
    _save(fig, Path(output_dir) / "figures" / "sector_distribution.png")


def plot_sequence_length(df: pd.DataFrame, output_dir: str) -> None:
    ...
    plt.tight_layout()
    _save(fig, Path(output_dir) / "figures" / "sequence_length_distribution.png")


def plot_drift_distribution(risk_df: pd.DataFrame, output_dir: str) -> None:
    ...
    ax.set_ylabel("Number of Users")
    plt.tight_layout()
    _save(fig, Path(output_dir) / "figures" / "career_drift_distribution.png")


def plot_transition_heatmap(transitions: pd.DataFrame, df: pd.DataFrame, output_dir: str) -> None:
    ...
    plt.yticks(fontsize=7)
    plt.tight_layout()
    _save(fig, Path(output_dir) / "figures" / "transition_heatmap.png")


def plot_top_transitions(transitions: pd.DataFrame, output_dir: str) -> None:
    top = (
    ...
    ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{int(x):,}"))
    plt.tight_layout()
    _save(fig, Path(output_dir) / "figures" / "top_transitions.png")


def plot_market_drift(market_drift: dict, output_dir: str) -> None:
    if not market_drift:
    ...
    ax.set_xlabel("Decade window"); ax.set_ylabel("Drift magnitude (Frobenius norm)")
    plt.xticks(rotation=30, ha="right")
    plt.tight_layout()
    _save(fig, Path(output_dir) / "figures" / "market_drift.png")


def plot_user_clusters(risk_df: pd.DataFrame, cs2: pd.DataFrame, output_dir: str) -> None:
    ...
    plt.tight_layout()
    _save(fig, Path(output_dir) / "figures" / "user_clusters.png")


def plot_cluster_sector_breakdown(risk_df: pd.DataFrame, cs2: pd.DataFrame,
                                  dom_sec_df: pd.DataFrame, output_dir: str) -> None:
    tsp = (
        risk_df[risk_df["cluster"].notna()]
    ...
    plt.tight_layout()
    _save(fig, Path(output_dir) / "figures" / "cluster_sector_breakdown.png")
...
def plot_accuracy_comparison(
    ...
    plt.tight_layout()
    _save(fig, Path(output_dir) / "figures" / "accuracy_comparison.png")


def plot_association_rules_lift(assoc_rules: pd.DataFrame, output_dir: str) -> None:
    ...
    ax.set_xlabel("Lift")
    plt.tight_layout()
    _save(fig, Path(output_dir) / "figures" / "association_rules_lift.png")
...
def run_all_plots(
    ...
    log.info("--- VISUALISATIONS ---")
    plot_sector_distribution(df, output_dir)
    plot_sequence_length(df, output_dir)
    plot_drift_distribution(risk_df, output_dir)
    plot_transition_heatmap(transitions, df, output_dir)
    plot_top_transitions(transitions, output_dir)
    plot_market_drift(market_drift, output_dir)
    plot_user_clusters(risk_df, cs2, output_dir)
    plot_cluster_sector_breakdown(risk_df, cs2, dom_sec_df, output_dir)
    plot_accuracy_comparison(
        ensemble_results,
        ensemble_results["persistence_top1"],
        output_dir,
    )
    plot_association_rules_lift(assoc_rules, output_dir)
...
def print_summary(
    ...
    log.info("    Bootstrap labels : regex map_sectors() + broad_map() (ported from R)")
    ...
    log.info("    Accuracy  : %.4f", nlp_metrics["accuracy"])
    log.info("    Precision : %.4f", nlp_metrics["precision"])
    log.info("    Recall    : %.4f", nlp_metrics["recall"])
    log.info("    F1 Score  : %.4f", nlp_metrics["f1"])
    log.info("  %s", "-" * 60)
    log.info("  Career Prediction (Ensemble)")
    log.info("    Persistence Baseline Top-1 : %.2f%%", ensemble_results["persistence_top1"] * 100)
    ...
    # Figures go to results/figures/, tables go to results/tables/
    OUTPUT_FILES = [
        ("figures", "nlp_confusion_matrix.png"),
        ("figures", "sector_distribution.png"),
        ("figures", "sequence_length_distribution.png"),
        ("figures", "pca_sector_profiles.png"),
        ("figures", "career_drift_distribution.png"),
        ("figures", "transition_heatmap.png"),
        ("figures", "top_transitions.png"),
        ("figures", "market_drift.png"),
        ("figures", "user_clusters.png"),
        ("figures", "cluster_sector_breakdown.png"),
        ("figures", "accuracy_comparison.png"),
        ("figures", "association_rules_lift.png"),
        ("tables",  "drift_risk_scores.csv"),
        ("tables",  "association_rules.csv"),
    ]
    log.info("Output files:")
    for subdir, fname in OUTPUT_FILES:
        exists = (Path(output_dir) / subdir / fname).exists()
        log.info("  %s  %s/%s", "✓" if exists else "✗", subdir, fname)
...
def main() -> None:
    cfg = parse_args()
    make_dirs(cfg)
    setup_file_logging(cfg)   # mirror all terminal output → results/logs/pipeline.log
    ...
    # ---------------------------------------------------------------------- 1
    ckpt_clean = Path(cfg["cache_dir"]) / "df_clean.parquet"
    if cfg["use_cache"] and ckpt_clean.exists():
        log.info("Loading cleaned data checkpoint: %s", ckpt_clean)
    ...
    # ---------------------------------------------------------------------- 2
    ckpt_reg = Path(cfg["cache_dir"]) / "df_regex.parquet"
    if cfg["use_cache"] and ckpt_reg.exists():
        log.info("Loading regex-labelled checkpoint: %s", ckpt_reg)
    ...
    # ---------------------------------------------------------------------- 3
    ckpt_nlp = Path(cfg["cache_dir"]) / "df_nlp_sectors.parquet"
    if cfg["use_cache"] and ckpt_nlp.exists():
        log.info("Loading NLP-sector checkpoint: %s", ckpt_nlp)
        df = pd.read_parquet(ckpt_nlp)
        log.info("  Loading NLP classifier checkpoint …")
        classifier     = joblib.load(Path(cfg["model_dir"]) / "lgbm_sector_classifier.joblib")
        embedding_model = SentenceTransformer("sentence-transformers/all-MiniLM-L6-v2")
        nlp_metrics    = {"accuracy": 0.0, "precision": 0.0, "recall": 0.0, "f1": 0.0}
        log.info("  NLP metrics not re-evaluated from cache (use --no-cache to recompute)")
    else:
        classifier, embedding_model, nlp_metrics = train_nlp_classifier(df, cfg)
        df = predict_nlp_sectors(df, classifier, embedding_model, cfg)
    ...
    # ---------------------------------------------------------------------- 6 — Ensemble
    log.info("--- ENSEMBLE PREDICTION SYSTEM ---")
    ...
    self_rates     = {s: float(sr_train.get(s, 0.2)) for s in all_sectors}
    self_loop_mult = {s: 1.0 + 2.0 * self_rates[s] for s in all_sectors}

    train_data, val_data, test_data, _ = stratified_user_split(df, transitions, cfg["seed"])

    # Re-index on training sectors (mirrors R exactly)
    all_sectors_tr  = sorted(set(train_data["esco_sector"]) | set(train_data["next_sector"]))
    ...
    self_rates_tr     = {s: float(sr_tr_s.get(s, 0.2)) for s in all_sectors_tr}
    ...
    # Annotate val and test with archetype + univ_flag + win_key
    for split_df in (val_data, test_data):
        split_df.update(
            split_df
            .merge(user_cluster_map[["person_id", "archetype"]], on="person_id", how="left")
        )
        split_df["archetype"] = split_df.get("archetype", pd.Series("Stable Specialist")).fillna("Stable Specialist")
        split_df["univ_flag"] = split_df["person_id"].map(person_univ).fillna(False)
        split_df["win_key"]   = split_df["year"].apply(
            lambda yr: get_win_key(yr, components["transition_matrices"])
        )

    # Merge archetype properly (merge returns new df)
    val_data = (
        val_data
    ...
    val_data["win_key"]   = val_data["year"].apply(
        lambda yr: get_win_key(yr, components["transition_matrices"])
    )
    ...
    test_data["win_key"]   = test_data["year"].apply(
        lambda yr: get_win_key(yr, components["transition_matrices"])
    )

    sector_best_w = tune_sector_weights(
        val_data, all_sectors_tr, sect_idx_tr, self_rates_tr,
        components, n_sec_tr, cfg["seed"],
    )
    ...
    risk_df, cs2, dom_sec_df = compute_drift_and_clusters(
        ...
        sector_to_idx, all_sectors, cfg["seed"], cfg["output_dir"],
    )

    # ---------------------------------------------------------------------- 8
    assoc_rules = compute_association_rules(transitions, cfg["output_dir"])
    ...
    if cfg.get("plots", True):
        run_all_plots(
            ...
            ensemble_results, cfg["output_dir"],
        )

    # --------------------------------------------------------------------- 10
    print_summary(nlp_metrics, ensemble_results, pca_var, cfg["output_dir"])
